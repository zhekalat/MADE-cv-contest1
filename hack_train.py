...
from hack_utils import AffineAugmenter, BrightnessContrastAugmenter, BlurAugmenter
from hack_utils import ThousandLandmarksDataset
from hack_utils import restore_landmarks_batch, create_submission, draw_landmarks
...
def main(args):
    ...
    optimizer = RAdam(model.parameters(), lr=args.learning_rate)  # , weight_decay=0.01)
    ...
    if args.draw:
        ...
        directory = os.path.join("result", test_dataset.image_names[0].split('.')[0])
        if not os.path.exists(directory):
            os.makedirs(directory)
        random_idxs = np.random.choice(len(test_dataset.image_names), size=1000, replace=False)
        for i, idx in enumerate(random_idxs, 1):
            image = cv2.imread(test_dataset.image_names[idx])
            image = draw_landmarks(image, test_predictions[idx])
            cv2.imwrite(os.path.join("result", test_dataset.image_names[idx]), image)
