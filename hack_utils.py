...
class AffineAugmenter(object):
    def __init__(self, min_scale=0.9, max_offset=0.1, rotate=True, elem_name='image'):
    ...
    def __call__(self, sample):
        ...
        sample[self.elem_name] = cv2.warpAffine(image, rotation, (w, h), borderValue=(128, 128, 128))

        if 'landmarks' in sample:
            landmarks = sample['landmarks'].reshape(-1, 2)
            x_new = landmarks[:, 0] * rotation[0, 0] + landmarks[:, 1] * rotation[0, 1] + rotation[0, 2]
            y_new = landmarks[:, 0] * rotation[1, 0] + landmarks[:, 1] * rotation[1, 1] + rotation[1, 2]
            landmarks = torch.stack((x_new, y_new), 1)
...
class BrightnessContrastAugmenter(object):
    ...
    def __call__(self, sample):
        ...
        image = image + (255 * brightness)
        image = (image - 128) * contrast + 128
        sample[self.elem_name] = image.clip(0, 255).astype(np.uint8)
        return sample
...
class BlurAugmenter(object):
    ...
    def __call__(self, sample):
        ...
        image = cv2.GaussianBlur(sample[self.elem_name], (kernel, kernel), sigmaX=kernel / 2)
        sample[self.elem_name] = image.clip(0, 255).astype(np.uint8)
        return sample
...
def create_submission(path_to_data, test_predictions, path_to_submission_file):
    ...
    for i, row in mapping.iterrows():
        ...
        needed_points = points_for_image[point_index_list].astype(np.int).clip(0, 2000)
